...
8. **Privacidade, LGPD e operação da autenticação**
   ...
   - o Core já possui processo administrativo via API/CLI com chaves com expiração/revogação, escopos e auditoria, além de login humano com senha + TOTP + sessão curta conectado às operações administrativas; falta construir o frontend e concluir a implantação operacional;
